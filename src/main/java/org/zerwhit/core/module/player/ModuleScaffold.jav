...
public class ModuleScaffold extends ModuleBase implements ITickableModule {
        ...
        if (target != null) {
            switch (stage) {
                ...
                case 1:
                    ...
                    mc.thePlayer.rotationPitch += RotationManager.wrapAngleDiff(pitch, getCurrentPitch()) * 0.2f;
                    mc.thePlayer.rotationYaw += RotationManager.wrapAngleDiff(yaw, getCurrentYaw()) * 0.2f;
                    ...
                    Hooks.transformMovementInput(mc.thePlayer.movementInput, yawDiff);
                    if ((deltaTime > 200L && !mc.thePlayer.onGround) && placeTick <= 0) {
                        ...
                        if (mc.thePlayer.onGround) flag = false;
                        if (flag)
    ...
    private boolean isForwardPressed() {
        return mc.gameSettings.keyBindForward.isKeyDown() || Keyboard.isKeyDown(mc.gameSettings.keyBindForward.getKeyCode());
    }
